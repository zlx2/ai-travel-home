...
- 包管理器：`npm`
- 开发端口：`5173`
- 默认联调：`.env.development` 中 `VITE_USE_MOCK=false`，请求会走后端 `/api`
- Mock 数据：保留在 `src/data/mock.ts`，用于后端接口未就绪时开发页面
...
- `VITE_USE_MOCK=true`：前端走 `src/data/mock.ts` 和 localStorage mock 数据。
- `VITE_USE_MOCK=false`：请求后端接口。
...
| 页面 | 路由 | 当前数据来源/状态 |
| --- | --- | --- |
| 首页 | `/` | 后端 `/home`；失败时静默回退到前端 mock 首页数据 |
| 登录 | `/login` | 后端 API |
| 注册 | `/register` | 后端 API |
| 个人中心 | `/profile` | Mock / API 双模式 |
| AI 行程规划 | `/ai-trip` | Mock / API 双模式；真实模式调用 `/api/ai/trips/analyze`、`/api/ai/trips/generate` |
| 我的行程 | `/trips` | Mock / API 双模式；后端行程 CRUD 仍可能是骨架 |
| 行程详情 | `/trips/:id` | Mock / API 双模式 |
| 游记列表 | `/notes` | Mock / API 双模式；后端游记接口仍可能是骨架 |
| 游记详情 | `/notes/:id` | Mock / API 双模式 |
| 写游记 | `/notes/create` | Mock / API 双模式 |
| 编辑游记 | `/notes/edit/:id` | Mock / API 双模式 |
...
```text
src/
├── api/             # API 调用层，内部根据 VITE_USE_MOCK 切换 mock/API
├── assets/          # 静态资源
...
- 首页 `/home` 做了静默 fallback，不会因为后端首页数据异常弹占位错误。
