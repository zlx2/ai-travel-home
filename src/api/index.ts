...
function normalizeTripDay(day:any,index:number){
  ...
  return{day:day.day||day.dayNo||index+1,title:day.title||day.theme||day.dailyTheme||`Day ${index+1}`,city:day.city,activities:activities.map((item:any,itemIndex:number)=>{const routeLeg=routeLegs.find((leg:any)=>Number(leg.fromOrder)===Number(item.order||itemIndex+1));return{time:item.time||item.startTime||item.arrivalTime||'',title:item.title||item.place||item.name||item.spotName||item.poiName||'',description:item.description||item.activity||item.summary||item.reason||item.recommendReason||'',tags:item.tags||[item.type,item.transport].filter(Boolean),cost:Number(item.cost??item.ticketCost??item.ticketPrice??item.estimatedCost??0),costText:item.costText||item.ticketCostText,suggestedDuration:item.suggestedDuration||item.suggestedDurationText||item.stayDuration||item.duration||item.recommendedStayTime||(item.suggestedDurationMinutes?`${item.suggestedDurationMinutes} 分钟`:undefined),suggestedDurationSource:item.suggestedDurationSource,transportSuggestion:cleanTransportSuggestion(item.transportSuggestion||routeLeg?.suggestion,rentalDay||String(routeLeg?.mode||'').includes('DRIVING')),reason:item.reason||item.recommendReason||item.recommendationReason||item.tips,area:item.area||item.region,address:item.address,city:item.city,lng:Number(item.entranceLng??item.lng??item.longitude)||undefined,lat:Number(item.entranceLat??item.lat??item.latitude)||undefined,openingHours:item.openingHours,rating:item.rating==null?undefined:Number(item.rating),averageCost:item.averageCost==null?undefined:Number(item.averageCost),businessArea:item.businessArea,imageUrls:item.imageUrls||[],type:item.type,order:item.order}}),timeline,startAnchor:normalizeAnchor(day.startAnchor),endAnchor:normalizeAnchor(day.endAnchor),food:day.food||day.foodSuggestions||day.diningSuggestions||day.restaurantSuggestions||[],budget:Number(day.budget||estimated.total||0),estimatedCost:{tickets:Number(estimated.tickets||0),food:Number(estimated.food||0),transport:Number(estimated.transport||0),total:Number(estimated.total||0),ticketSource:estimated.ticketSource,foodSource:estimated.foodSource,transportSource:estimated.transportSource,excludesUnknownItems:estimated.excludesUnknownItems},intensity:day.intensity||day.pace||day.tripIntensity,accommodation:day.accommodation||day.accommodationArea||day.hotelArea,diningArea:day.diningArea||day.foodArea||day.restaurantArea,routeSummary:day.routeSummary||day.route,tips:day.tips||day.dayTips||[]}
}
function normalizeTimelineNode(item:any,index:number){
  return{time:item.time||item.startTime||'',title:item.title||item.name||'',subtitle:item.subtitle,description:item.description||item.reason||item.subtitle||'',tags:item.tags||[item.type].filter(Boolean),cost:Number(item.cost??item.estimatedCost??0),costText:item.costText,suggestedDuration:item.suggestedDuration||item.durationText||(item.durationMinutes?`${item.durationMinutes} 分钟`:undefined),transportSuggestion:cleanTransportSuggestion(item.transportSuggestion),reason:item.reason||item.description,area:item.area,address:item.address,city:item.city,lng:Number(item.lng??item.longitude)||undefined,lat:Number(item.lat??item.latitude)||undefined,type:item.type,compact:item.compact!==false,order:item.order??index+1}
}
function normalizeAnchor(anchor:any){
  if(!anchor)return undefined
  return{type:anchor.type,name:anchor.name,city:anchor.city,area:anchor.area,address:anchor.address,lng:Number(anchor.lng)||undefined,lat:Number(anchor.lat)||undefined,coordType:anchor.coordType}
}
...
export const rentalApi={
  previewContext(payload:{requirement:any;arrivalText?:string}){return request.post('/rental/context/preview',payload,{suppressError:true}) as Promise<any>},
  ...
  alipayPagePay(id:number){return request.post(`/rental/orders/${id}/alipay/page-pay`) as Promise<{orderId:number;orderNo:string;formHtml:string}>},
