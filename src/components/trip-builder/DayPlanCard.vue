...
<template>
  <article class="day-plan-card builder-card">
    ...
    <section class="metric-strip">
      ...
      <div><span>预计费用</span><b>¥ {{ day.budget.total }}</b></div>
      ...
      <div class="timeline-panel">
        <div v-for="(moment,index) in day.moments" :key="moment.key" class="moment-row" :class="{ expanded: expandedKey===moment.key }">
          ...
          <div v-if="expandedKey===moment.key" class="moment-detail">
            <div><b>建议停留</b><span>{{ index===1 ? '约 1 小时' : index===3 ? '约 1.5 小时' : '约 2 小时' }}</span></div>
            <div><b>交通建议</b><span>{{ day.rental.enabled ? '自驾或短途打车，提前确认停车点。' : '优先步行或短途打车，减少换乘。' }}</span></div>
            <div><b>体验重点</b><span>保留核心体验，同时预留休息和拍照时间，避免连续赶路。</span></div>
          </div>
        </div>
      </div>
    </section>
