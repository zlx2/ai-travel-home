...
const ROUTE_HEALTH_RULES=[
  {level:'danger',max:45,title:'当天路程偏长',text:'这一天跨区距离较大，更适合拆分或点击“调整路线”重新规划。'},
  {level:'warn',max:32,title:'交通时间较多',text:'建议确认是否接受较长打车时间。'},
]
const LIGHT_ROUTE_HEALTH_RULE={
  level:'warn',
  max:24,
  title:'轻松游距离偏高',
  text:'当前路线可能不够松弛，建议减少跨区移动。',
}
const TEXT_REPLACEMENTS=[
  ['高德路线暂不可用，前端将重新计算。','建议打车衔接，出发前按实时路况调整。'],
  ['坐标不足，路线待前端计算。','建议按当天实际位置灵活选择步行或打车。'],
]
...
const routeHealth=computed(()=>{
  ...
  if(props.day.intensity==='LIGHT'&&mileage>LIGHT_ROUTE_HEALTH_RULE.max)return LIGHT_ROUTE_HEALTH_RULE
...
const displayCost=(moment:{costText?:string;cost:number})=>{
  if(moment.costText)return moment.costText.replace('门票待确认','门票以现场为准')
  return moment.cost>0?`门票参考 ¥${moment.cost}/人`:'门票以现场为准'
}
const displayOpeningHours=(value:string)=>{
  const normalized=value.replace(/\s+/g,' ').trim()
  const first=normalized.split(/[;；]/)[0]
  return first.length>36?`${first.slice(0,36)}...`:first
}
const displayTransport=(value:string|undefined,fallback:string)=>{
  if(!value)return fallback
  return TEXT_REPLACEMENTS.reduce((text,[from,to])=>text.replace(from as string,to as string),value)
}
  ...
  <article class="day-plan-card builder-card">
    ...
    <section class="metric-strip">
      ...
      <div class="cost-metric">
        <span>花费参考</span><b>¥ {{ day.budget.total }}</b>
        <small>{{ day.budget.excludesUnknownItems ? '不含住宿与未确认门票' : '餐饮与市内交通' }}</small>
      </div>
      ...
      <div class="timeline-panel">
        <div v-for="(moment,index) in day.moments" :key="moment.key" class="moment-row" :class="{ expanded: expandedKey===moment.key }">
          ...
          <div v-if="expandedKey===moment.key" class="moment-detail">
            <div><b>建议停留</b><span>{{ moment.suggestedDuration || '约 1-2 小时' }}</span></div>
            <div v-if="moment.openingHours"><b>营业时间</b><span>{{ displayOpeningHours(moment.openingHours) }}</span></div>
            ...
            <div><b>交通建议</b><span>{{ displayTransport(moment.transportSuggestion, day.rental.enabled ? '自驾或短途打车，提前确认停车点。' : '优先步行或短途打车，减少换乘。') }}</span></div>
