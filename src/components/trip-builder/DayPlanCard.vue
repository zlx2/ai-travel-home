...
const isMapMoment=(moment:DayMoment)=>!isMealMoment(moment)&&typeof moment.lng==='number'&&typeof moment.lat==='number'
const mapMomentNumber=(index:number)=>props.day.moments.slice(0,index+1).filter(isMapMoment).length
    ...
    <section v-else class="timeline-scroll">
      <div class="timeline-panel">
        ...
        >
          ...
          <div class="moment-pin" :class="{ muted: !isMapMoment(moment) }">{{ isMapMoment(moment) ? mapMomentNumber(index) : '' }}</div>
...
.moment-pin.muted{background:transparent!important;border:0;box-shadow:none;color:transparent}
.meal-card{border-left:0;padding-left:0}
.meal-card-head{align-items:flex-start}
.meal-icon{margin-top:1px;background:#fff7e6}
</style>
