...
const knownTotal = () => tripCost() + (props.data.hotelCost || 0) + rentalCost()
  ...
  <section class="final-review">
    ...
    <div class="review-grid">
      ...
      <aside class="pay-card review-card">
        <h3>订单概览</h3>
        <div class="fee-lines">
          <p><i class="green"><el-icon><Wallet /></el-icon></i><span>行程内费用</span><b>¥{{ tripCost() }}</b></p>
          <p><i class="blue"><el-icon><Briefcase /></el-icon></i><span>住宿费用</span><b>{{ data.hotelCost == null ? '待确认' : `¥${data.hotelCost}` }}</b></p>
          <p><i class="purple"><el-icon><Van /></el-icon></i><span>租车费用</span><b>¥{{ rentalCost() }}</b></p>
        </div>
        <div class="total-line"><span>已知费用小计</span><strong>¥{{ knownTotal() }}</strong></div>
        <ul class="safe-list">
        ...
        <button class="pay-primary" type="button" :disabled="saving || paid" @click="orderCreated ? emit('sandboxPay') : emit('createOrder')">
          ...
          {{ paid ? '支付已完成' : orderCreated ? `确认支付　¥${knownTotal()}` : `确认支付　¥${knownTotal()}` }}
        </button>
