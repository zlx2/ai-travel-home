...
import type { AnalyzeQuestion, Destination, Requirement } from '../../types'
...
const emit=defineEmits<{
  ...
  'update:followUpAnswer':[field:string,value:string]
  analyze:[]
  submitFollowUp:[]
...
<template>
  <section class="nl-card builder-card">
    ...
    <div class="input-layout">
      ...
      <div v-if="questions?.length" class="follow-up-panel">
        <div class="follow-up-head">
          <strong>还差一点信息</strong>
          <small>补充下面的问题后，PlanGo 会继续分析需求。</small>
        </div>
        <div class="follow-up-list">
          <label v-for="item in questions" :key="item.field">
            <span>{{ item.question }}</span>
            <el-input
              :model-value="followUpAnswers[item.field]||''"
              :placeholder="item.field==='days'?'例如：3天':'直接填写即可'"
              @update:model-value="emit('update:followUpAnswer',item.field,$event)"
              @keyup.enter="emit('submitFollowUp')"
            />
          </label>
        </div>
        <el-button class="builder-primary" type="primary" :loading="loading" @click="emit('submitFollowUp')">
          提交补充信息
        </el-button>
      </div>
