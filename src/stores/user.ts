...
export const useUserStore=defineStore('user',()=>{
  ...
  async function login(account:string,password:string){const data=await authApi.login({account,password});token.value=data.token;user.value=data.user;localStorage.setItem(TOKEN_KEY,data.token);localStorage.setItem(USER_KEY,JSON.stringify(data.user));return data.user}
  async function logout(){try{await authApi.logout()}catch{/* 服务端会话失效时也要清理本地登录态。 */}token.value='';user.value=null;clearAuthStorage()}
