...
export interface Activity { time:string; title:string; description:string; tags:string[]; cost:number; costText?:string; suggestedDuration?:string; suggestedDurationSource?:string; transportSuggestion?:string; reason?:string; area?:string; address?:string; city?:string; lng?:number; lat?:number; openingHours?:string; rating?:number; averageCost?:number; businessArea?:string; imageUrls?:string[]; type?:string; compact?:boolean; order?:number; subtitle?:string }
export interface TripAnchor { type?:string; name?:string; city?:string; area?:string; address?:string; lng?:number; lat?:number; coordType?:string }
...
export interface TripDay { day:number; title:string; city?:string; activities:Activity[]; timeline?:Activity[]; startAnchor?:TripAnchor; endAnchor?:TripAnchor; food:string[]; budget:number; estimatedCost?:DayEstimatedCost; intensity?:string; accommodation?:string; diningArea?:string; routeSummary?:string; tips?:string[] }
...
export interface Trip { id:number; userId?:number; username?:string; title:string; departure?:string; destination:string; days:number; budget:number; preferences:string[]; summary:string; coverUrl?:string; requirementJson?:Requirement; tripPlanJson:TripPlan; status:number; createTime:string; updateTime?:string }
